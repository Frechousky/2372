...
def test_on_left_arrow_input_decrement_cursor_pos(
    ...
    assert tested._model.cursor_pos == CURSOR_POS_INIT, "cursor_pos has it's init value"
...
def test_on_enter_input_init_i18n(mocker, tested: LocaleSelectionInputHandler):
    mock_ret = [pygame.event.Event(pygame.KEYDOWN, {"key": pygame.K_RETURN})]
    mocker.patch("pygame.event.get", return_value=mock_ret)
    mocker.patch("scenes.locale_selection_scene.MenuScene")
    assert (
        i18n.get("locale") != LOCALES[tested._model.cursor_pos]
    ), "locale is set before pressing enter"
    tested.handle_inputs()
    assert (
        i18n.get("locale") == LOCALES[tested._model.cursor_pos]
